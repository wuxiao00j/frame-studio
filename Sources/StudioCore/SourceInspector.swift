...
public enum SourceInspector {
    ...
    public static func inventory(_ directory:URL)throws->[String:Any] {
        ...
        var sources:[String]=[],assets:[String]=[],metadata:[String]=[]
        for case let f as URL in iterator {
            ...
            if ["Info.plist","InfoPlist.strings","project.pbxproj"].contains(f.lastPathComponent) || f.pathExtension=="xcstrings" {metadata.append(f.path)}
            else if sourceExtensions.contains(f.pathExtension.lowercased()){sources.append(f.path)}
            else if ["png","jpg","jpeg","webp","svg","heic"].contains(f.pathExtension.lowercased()){assets.append(f.path)}
            if sources.count+assets.count+metadata.count>=2000{break}
        }
        return ["root":directory.path,"sourceFiles":sources.sorted(),"assets":assets.sorted(),"metadataFiles":metadata.sorted(),"limit":2000,"instructions":"Use read_ui_source to inspect selected UI files. Reconstruct editable DesignProject pages/nodes through create_page/add_component/update_component or replace_project. Flutter/React layouts are agent-assisted. Original files are read-only; export creates a separate UI-source folder."]
    }
