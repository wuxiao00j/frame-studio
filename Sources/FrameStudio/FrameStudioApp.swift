    ...
    var body:some Scene {
        WindowGroup("原境 · Frame Studio") {EditorRoot(session:session).frame(minWidth:1180,minHeight:740).preferredColorScheme(.light)}
            ...
            .commands{
                ...
                CommandMenu("设计"){Button("复制组件",action:session.duplicate).keyboardShortcut("d").disabled(session.selection.isEmpty);Button("删除组件",action:session.deleteSelection).keyboardShortcut(.delete,modifiers:[.command]).disabled(session.selection.isEmpty);Button("合并组件",action:session.group).keyboardShortcut("g").disabled(session.selection.count<2);Button("拆分组合",action:session.ungroup).keyboardShortcut("g",modifiers:[.command,.shift]);Button("上一层"){session.reorder(.forward)}.keyboardShortcut("]",modifiers:.command).disabled(!session.canReorder(.forward));Button("下一层"){session.reorder(.backward)}.keyboardShortcut("[",modifiers:.command).disabled(!session.canReorder(.backward));Button("切换横竖屏",action:session.rotate).keyboardShortcut("r",modifiers:[.command,.shift]);Button("预览交互"){session.preview.toggle()}.keyboardShortcut("p",modifiers:[.command,.shift])}
            }
    }
}
