...
@MainActor struct TemplateComposer:View {
    ...
    var body:some View {
        VStack(alignment:.leading,spacing:18){Text("创建组合组件").font(.title2.bold());Text("把选中的 \(session.templateSelectionNodes.count) 个图层保存为可重复使用的组件。").font(.system(size:12)).foregroundStyle(studioMuted);TextField("组合组件名称",text:$session.templateName).textFieldStyle(.roundedBorder);HStack{TextField("分类（可自定义）",text:$session.templateCategory).textFieldStyle(.roundedBorder);Menu("选择分类"){ForEach(TemplateCatalog.categories,id:\.self){category in Button(category){session.templateCategory=category}}}};Toggle("同时保存到个人组件库（所有项目可用）",isOn:$session.saveTemplatePersonally);Text("插入后可以整体拖动，也能取消组合后逐个修改。所有布局和上传图片都会保留。").font(.system(size:11)).foregroundStyle(studioMuted);HStack{Button("取消"){dismiss()};Spacer();Button("保存组件"){session.confirmTemplate()}.buttonStyle(.borderedProminent).keyboardShortcut(.defaultAction)}}.padding(26).frame(width:430)
    }
}
