# 原境 · Frame Studio v1.2.0-beta.4

## 中文 · Beta 测试版

公开 Beta 更新（包含之前版本功能）：原生 SwiftUI Mac UI 设计器，支持 43 种组件、长页面、标准 / 阔屏布局、图标上传、Tab 双态图标、自定义组合，以及旧 UI 源码分类导入。

本次修复选择组件后点击或修改 X / Y / W / H 输入框的闪退，并修复进度样式切换、复制布局的同类数据访问冲突。新增直接调用编辑器属性绑定的回归测试；保留 Tab 共用配置、返回键和拆分解组功能。

**导出只包含 UI 源码、资源、项目配置和设计文件，不生成 APK、AAB、IPA 或可安装手机 App。** 原型导航、开关和表单不代表业务功能已完成，后续仍需开发真实业务逻辑。

- 下载 `FrameStudio-v1.2.0-beta.4-macOS-arm64.zip`，解压并将应用放到“应用程序”。
...
This update fixes a crash when focusing or editing X / Y / W / H after selecting a component, and the same data-access conflict in progress-style changes and layout copying. Regression tests now exercise the actual editor bindings. Shared Tabs, back buttons and automatic ungrouping remain available.
...
- Download `FrameStudio-v1.2.0-beta.4-macOS-arm64.zip`, unzip it and move the application into Applications.
